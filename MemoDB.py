...
load_dotenv()  # os.environ 설정 전에 호출
...
# Function to save non-link input
...
def retriv_one(query):
    ...
    docs = retriever.invoke(query)  # get_relevant_documents() → invoke() (deprecated 대응)
...
def retrieval_answer(query, db):
    ...
    result = qa.invoke({"query": query})  # __call__ → invoke() (deprecated 대응)
...
def generate_response(intent, passage):
    if intent is None:
        ...
        return ['Your memo is below.', '============================='] + \
               ['-' + memo for memo in db.get(where_document={'$contains': ' '})['documents']] + \
               ['=============================']
...
try:
    if "messages" not in st.session_state.keys():
        st.session_state.messages = [
            {"role": "assistant",
             "content": "I'm your DB manager. You can <save>, <del>, <qa>, <show>, <update>, or <summarize> memos by chatting 😃"}
        ]

    if prompt := st.chat_input("Query MemoDB"):
        st.session_state.messages.append({"role": "user", "content": prompt})

    for split in split_session_state(st.session_state.messages):
        with st.chat_message(split[0]["role"]):
